...
Toute modification doit être faite dans une branche dédiée créée à partir de `main` et nommée selon les conventions suivantes :
...
1.  **Création** : Ouvrez une PR de votre branche vers `main`.
...
4.  **Fusion (Merge)** : Utilisez de préférence le mode **Squash and Merge** pour garder un historique `main` propre et linéaire.
